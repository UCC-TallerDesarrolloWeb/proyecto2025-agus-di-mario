...
import { obtenerResena, guardarResena } from '@api/albumes'
...
function SeccionResena({ album }) {
  ...
  /**
   * Actualiza el texto y valida en vivo.
  ...
  /**
   * Actualiza el puntaje y ejecuta validación inmediata.
  ...
  /**
   * Valida y guarda la reseña actual.
   * @returns {void}
   */
    ...
    <div className="seccion-resena">
      {estaEditando ? (
        <div className="entradas-resena">
          <label htmlFor={idResena} className="sr-only">Escribe tu reseña breve</label>
          <textarea
            id={idResena}
          ...
          />
          {errores.texto && (
            <p className="mensaje-error" role="alert" id={`${idResena}-error`} aria-live="polite">{errores.texto}</p>
          )}
          <div className="contador-caracteres">{contador}</div>
          <label htmlFor={idPuntaje} className="sr-only">Selecciona un puntaje del 1 al 10</label>
          <select
            id={idPuntaje}
          ...
          {errores.puntaje && (
            <p className="mensaje-error" role="alert" id={`${idPuntaje}-error`} aria-live="polite">{errores.puntaje}</p>
          )}
          <Boton type="button" className="boton-accion" onClick={manejarGuardarResena}>Guardar</Boton>
        </div>
      ) : (
        <div className="resena-guardada">
          ...
          <Boton type="button" variante="secundario" className="boton-accion" onClick={manejarEditarResena}>Editar</Boton>
        </div>
      )}
    </div>
  )
